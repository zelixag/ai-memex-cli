...
export const zhCN = {
  ...
  scenarios: {
    ...
    sectionSubtitle:
      "memex 的日常入口应该在 agent 对话里。你告诉 agent 要 capture、query、distill、ingest 或 lint；CLI 只在背后承担确定性的机械工具。",
    readArticle: "阅读场景",
...
你：/memex:lint 修复断链和 stale frontmatter，语义合并前先问我
你：/memex:query "open questions stale pages duplicate concepts"`,
        body: [
          {
            heading: "怎么使用",
            text:
              "把 agent 当成 wiki maintainer，而不是问答机器人。你用自然语言或 slash prompt 让它 capture、ingest、query、distill、lint、status；agent 再决定背后调用哪些 memex 机械原语。",
            steps: [
...
你：告诉我改了什么，还有哪些需要 review。`,
          },
          {
            ...
            code: `你：/memex:lint 知识库结构。
Agent：运行机械层 lint/link，做语义层扫描，提出修复计划，并在高风险合并前询问。
...
memex context status`,
  ...
  architecture: {
    ...
    vaultTree: [
      ...
      { indent: 3, text: "comparisons/", note: "并列对比" },
      { indent: 3, text: "overviews/", note: "领域/项目横扫" },
      { indent: 3, text: "syntheses/", note: "派生洞察与连接" },
    ],
  ...
  commands: {
    ...
    util: {
      ...
      new: {
        ...
        flags: ["<type>  entity | concept | source | comparison | overview | synthesis", "<name>  页面名称"],
      },
      log: {
      ...
      migrate: {
        desc: "将旧 schema wiki 页迁移到新的 6-type 架构。",
        usage: "memex migrate --from-summary-subtype --dry-run\nmemex migrate --from-summary-subtype",
        flags: [
          "--from-summary-subtype  将 type:summary+subtype 转为 6 种平级 type",
          "--dry-run               预览改动，不实际修改文件",
          "--vault <path>          Vault 路径",
        ],
      },
      update: {
